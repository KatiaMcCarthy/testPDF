namespace AndroidGoodiesExamples
{
	...
	{
#if UNITY_ANDROID
		const string CustomDataKey1 = "customKey1";
		const string CustomDataKey2 = "customKey2";
		...
		void Start()
		{
			...
			{
				_resultText.text = string.Format("App was opened via notification with parameters: {0}, {1}",
		...
		{
			Notify(CreateBaseNotificationBuilder());
		}
		...
		{
			var notification = CreateBaseNotificationBuilder().Build();
			var time = DateTime.Now;
			Debug.Log("NotificationsTest: time is " + time);
...
#pragma warning disable 0618
			var builder = new Notification.Builder(channelId, new Dictionary<string, string>
				{
					{CustomDataKey1, "value1"},
					{CustomDataKey2, "value2"}
				})
				.SetGroup(groupId)
...
#pragma warning restore 0618
		...
		public void CreateDifferentNotifications()
		{
			...
			builder = CreateBaseNotificationBuilder().SetNumber(20);
			Notify(builder);

			//Chronometer notification
			builder = CreateBaseNotificationBuilder().SetUsesChronometer(true);
			Notify(builder);

			//Notification with ticker and subtext
			builder = CreateBaseNotificationBuilder().SetSubText("Some subtext").SetTicker("Accessibility text");
			Notify(builder);

			//Auto-canceling notification (20s)
			builder = CreateBaseNotificationBuilder().SetTimeoutAfter(20L * 1000L);
			Notify(builder);

			//Ongoing notification
			builder = CreateBaseNotificationBuilder().SetOngoing(true);
			Notify(builder);

			//One-time alerting scheduled repeating notification
			builder = CreateBaseNotificationBuilder().SetOnlyAlertOnce(true);
